...
public class LocationCheckerJob extends JobService {
    ...
    void checkIfMoving() {
        currentX = Double.valueOf(getSharedPreferences("PREFERENCE", MODE_PRIVATE).getString("Current X", ""));
        currentY = Double.valueOf(getSharedPreferences("PREFERENCE", MODE_PRIVATE).getString("Current Y", ""));
                        ...
                        public void onResponse(Call<GPSCoordinates> call, Response<GPSCoordinates> response) {
                            if (response.isSuccessful()) {
                                if (response.raw().networkResponse() != null && response.raw().networkResponse().code() == HttpURLConnection.HTTP_NOT_MODIFIED) {
                                    ...
                                    getSharedPreferences("PREFERENCE", MODE_PRIVATE).edit().putString("Current X", Double.toString(currentX)).apply();
                                    getSharedPreferences("PREFERENCE", MODE_PRIVATE).edit().putString("Current Y", Double.toString(currentY)).apply();
                                }
                                checkIfMoving();
                            }
                        }
